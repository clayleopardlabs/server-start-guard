...
export function readConfig() {
    ...
    try {
        if (existsSync(CONFIG_PATH)) {
            ...
            if (typeof raw.defaultHealthChecks === "boolean")
                cfg.defaultHealthChecks = raw.defaultHealthChecks;
            if (typeof raw.healthTimeoutMs === "number" && raw.healthTimeoutMs > 0)
                cfg.healthTimeoutMs = raw.healthTimeoutMs;
            if (typeof raw.healthIntervalMs === "number" && raw.healthIntervalMs > 0)
                cfg.healthIntervalMs = raw.healthIntervalMs;
            if (typeof raw.healthGraceMs === "number" && raw.healthGraceMs > 0)
                cfg.healthGraceMs = raw.healthGraceMs;
        }
    }
    catch {
